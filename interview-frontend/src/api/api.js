...
export const MapUserProcedureToPlan = async (planId, procedureId, userId, deleted) => {
    ...
    if (!response.ok) throw new Error("Failed to create user to Plan");
...
export const getUsersPlanProcedures = async (planId) => {
    const url = `${api_url}/Plan/UserPlanProcedure?PlanId=${planId}`;
    const response = await fetch(url, {
        method: "GET",
    });
